...
void ComportamientoJugador::Nivel4(Sensores sensores)
{
	...
	if ((sensores.terreno[0] == 'G') and !current_stateN4.bienSituado)
	{
...
{
	int i0 = st.jugador.f, j0 = st.jugador.c;
	mapa[i0][j0] = terreno[0];
	for (int i = 1; i < terreno.size(); ++i)
	{
		int fil = i0 + casillasTerreno[static_cast<int>(st.jugador.brujula)][i].first;
		int col = j0 + casillasTerreno[static_cast<int>(st.jugador.brujula)][i].second;
...
list<Action> DijkstraSoloJugador(const stateN0 &inicio, const ubicacion &final, const vector<vector<unsigned char>> &mapa)
{
	...
	// i = fila, j = columna, k = orientacion, l = 0 nada, 1 bikini, 2 zapatillas
	...
	while (!fin)
	{
		...
		if (mapa[actual.st.jugador.f][actual.st.jugador.c] == 'K')
		{
			...
			else
			{
				...
				// if (!cerrados[childCogeBikini.st.jugador.f][childCogeBikini.st.jugador.c][childCogeBikini.st.jugador.brujula][1])
		...
		if (mapa[actual.st.jugador.f][actual.st.jugador.c] == 'D')
		{
			...
			else
			{
				...
				// if (!cerrados[childCogeZapatillas.st.jugador.f][childCogeZapatillas.st.jugador.c][childCogeZapatillas.st.jugador.brujula][2])
		...
		else
		{
			...
			// cout << "Find(Cerrados, Forward): " << Find(cerradosList,childForward) << endl;
			// cout << "cerrados[][][]: " << cerrados[childForward.st.jugador.f][childForward.st.jugador.c][childForward.st.jugador.brujula][l]<< endl;
			// if (!cerrados[childForward.st.jugador.f][childForward.st.jugador.c][childForward.st.jugador.brujula][l])
			if (!Find(cerrados, childForward))
			{
			...
			// if (!cerrados[childTurnL.st.jugador.f][childTurnL.st.jugador.c][childTurnL.st.jugador.brujula][l])
			...
			// if (!cerrados[childTurnR.st.jugador.f][childTurnR.st.jugador.c][childTurnR.st.jugador.brujula][l])
